...
<div class="toolbar" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-5 align-items-center my-1"><span class="text-muted fw-normal">Home - Course - Course Module Konten - </span>&nbsp;List Course Modul Konten</h1>
        </div>
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <a href="{{url('/back-mentor/course-module-content/add-course-module-content')}}" class="btn btn-sm btn-primary">Tambah Data</a>
        </div>
    </div>
</div>
@endsection
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
                <thead>
                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th class="text-center">Judul Course Konten</th>
                ...
                <tbody class="fw-bold text-gray-600">
                    @foreach ($getCourseModuleContent as $item)
                    <tr>
                        <td>
                        ...
                        <td class="text-center">
                            <span class="fw-bolder">{{$item->module_name}}</span>
                        </td>
                        <td class="text-center min-w-100px">
                            <span class="fw-bolder">{{$item->title_module_content}}</span>
                        </td>
                        <td class="text-center min-w-100px">
                            <span class="fw-bolder">{{$item->video_link}}</span>
                        </td>
                        <td class="text-center" data-order="{{$item->pdf_file}}">
                            <span class="fw-bolder">{{$item->pdf_file}}</span>
                        </td>
                        <td class="text-center min-w-100px">{{ \Illuminate\Support\Str::limit($item->description, 25, $end='...') }}</td>
                        <td class="text-center min-w-100px">{{$item->ordinal}}</td>
                        <td class="text-center">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-mentor/course-module-content/'.$item->id.'/edit-course-module-content')}}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="btn w-100 menu-link px-3 fs-7 text-center">Edit</button>
                                    </form>
                                </div>
                                <div class="menu-item px-3">
                                    <form action="{{url('/back-mentor/course-module-content/'.$item->id.'/destroy-course-module-content')}}" method="POST" class="inline">
                                        @method('delete')
                                        @csrf
